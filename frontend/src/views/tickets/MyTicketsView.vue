...
async function handleFilter() {
  await ticketStore.fetchMyTickets(1, pagination.value.pageSize, filter.status || undefined)
}

async function clearFilter() {
  filter.status = ''
  filter.keyword = ''
  await ticketStore.fetchMyTickets(1, pagination.value.pageSize)
}

async function handleSizeChange(size: number) {
  await ticketStore.fetchMyTickets(1, size, filter.status || undefined)
}

async function handlePageChange(page: number) {
  await ticketStore.fetchMyTickets(page, pagination.value.pageSize, filter.status || undefined)
}
</script>
